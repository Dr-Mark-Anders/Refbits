...
{
    [Serializable]
    public class ReformerBed : UnitOperation, ISerializable
    {
        ...
        private DistPoints Feed = new DistPoints();

        private Tuple<double, double, double, double> PNAO;
        private Components FeedComponents = new Components();

        public ReformerBed(string Name = "") : base()
        {
            this.Name = Name;
            Add(DP);
            Add(PortIn);
        ...
        public ReformerBed(SerializationInfo info, StreamingContext context) : base(info, context)
        {
        ...
        public void Initialise()
        {
            ...
            Feed.Add(new DistPoint(5, new Temperature(114, TemperatureUnit.Celsius)));
            ...
            {
                FeedComponents.Add(new PseudoComponent(Names[i], NBP[i], SG[i], MW[i], W[i], TCrit[i], PCrit[i], MolVOl[i], HFORM25[i], Cp[i]));
            }
        }
    }
}