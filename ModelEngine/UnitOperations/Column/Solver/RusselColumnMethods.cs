...
namespace RusselColumn
{
    ...
    public partial class RusselSolver
    {
        ...
        public void InitialiseSpecs(Column column)
        {
        ...
        public void FinishoffCondenser()
        {
        ...
        {
            UpdateIteration1?.Invoke(this, new IterationEventArgs(update));
        }

        public void DoUpdateIteration2(string update)
        {
            UpdateIteration2?.Invoke(this, new IterationEventArgs(update));
        }
        ...
        public void SetInitialTrayTemps(Column cd)
        {
            ...
            if (feed != null && column != null)
            {
                ...
                for (int sectionNo = 0; sectionNo < cd.TraySections.Count; sectionNo++)
                    ...
                    {
                        Tray tray = cd.TraySections[sectionNo].Trays[trayNo];
                        tray.T = t;
                    }
            }
        }

        public void InitialiseMatrices(Column column)
        {
            ...
            foreach (ConnectingStream stream in column.ConnectingStreamsAll)
            {
        ...
        public bool InitialiseFeeds(Column column)
        {
            TotalFeeds = 0;
            ...
            foreach (TraySection section in column.TraySections)
            {
                for (int TrayNo = 0; TrayNo < section.Trays.Count; TrayNo++) // do main column first
                {
                    ...
                    if (tray.feed != null && tray.feed.IsFullyDefined)
                    {
            ...
            for (int CompNo = 0; CompNo < column.Components.Count; CompNo++)
            {
                ...
                for (int sectionNo = 0; sectionNo < column.TraySections.Count; sectionNo++)
                {
                    ...
                    for (int trayNo = 0; trayNo < section.Trays.Count; trayNo++)
                    {
                        ...
                        if (feed.IsFullyDefined && (feed.IsConnected || feed.MolarFlow_ != 0)
                            && feed.cc != null
        ...
        public void InitialiseTrayCompositions(Column column)
        {
            if (MainFeed == null)
                return;
        ...
        public static void InitialisePumpArounds(Column column)
        {
        ...
        public static void InitialiseStrippers(Column column)
        {
            ConnectingStreamCollection LiquidStreams = column.LiquidStreams;

            foreach (ConnectingStream ds in LiquidStreams) // do liquid int ersection streams
            {
        ...
        public void Update_FlowFactors(Column column)
        {
        ...
        public static void UpdatePAdraws(Column column)
        {
        ...
        public static void UpdateConnectedSideDraws(Column column)
        {
            ...
            foreach (ConnectingStream cs in column.LiquidStreams)  // do liquid draws // TrDMatrix[Comp][Down][Across]
                if (cs.DrawTrayIndex >= 0)
                    cs.FlowEstimate = column.MainTraySection.Trays[cs.DrawTrayIndex].L * cs.DrawFactor;

            foreach (ConnectingStream cs in column.VapourStreams)  // do liquid draws // TrDMatrix[Comp][Down][Across]
                if (cs.DrawTrayIndex >= 0)
                    cs.FlowEstimate = column.MainTraySection.Trays[cs.DrawTrayIndex].V * cs.DrawFactor;
        }

        public static void UpdateSideNetReturns(Column column)  // e.g. for mole balance
        {
            ...
            foreach (ConnectingStream st in column.ConnectingNetVapourStreams)
            {
            ...
            foreach (ConnectingStream st in column.ConnectingNetLiquidStreams)
            {
        ...
        public void ResetFlowValues()
        {
            ...
            foreach (ConnectingStream st in column.ConnectingNetVapourStreams)
            {
                st.FlowEstimate = st.InitialFlowEstimate;
            }
        ...
        public void InitialiseFlowsSimplified(Column column)
        {
            ResetFlowValues();
            ...
            foreach (SideStream ss in column.LiquidSideStreams)
            {
                ...
                {
                    Specification s = column.Specs.GetSpecs(ss).GetLiquidDrawSpec(ss.EngineDrawSection.Guid, ss.EngineDrawTray.Guid);
                    if (s != null)
            ...
            foreach (SideStream ss in column.VapourSideStreams)
            {
                if (ss.EngineDrawSection != null)
                {
                    Specification s = column.Specs.GetSpecs(ss).GetVapourDrawSpec(ss.EngineDrawSection.Guid, ss.EngineDrawTray.Guid);
                    if (s != null)
        ...
        public void InitialiseFlows(Column column)
        {
        ...
        public void InitialiseFlows2(Column column)
        {
            ResetFlowValues();
            ...
            foreach (SideStream stream in column.SideStreamsAll())
            {
                if (stream.EngineDrawSection != null && stream.EngineDrawTray != null)
                {
                    SpecificationCollection sc = column.Specs.GetActiveSpecs().GetSpecs(stream);

                    foreach (Specification s in sc)
                    {
                        switch (s.engineSpecType)
                        {
                            case eSpecType.LiquidProductDraw:
            ...
            foreach (TraySection section in column.TraySections)
            {
                ...
                switch (section.CondenserType)
                {
                    case CondType.Partial: // vapour can be condensed
                    case CondType.TotalReflux:
                        vaprate = column.Specs.GetActiveSpecs().GetCondVapourRateSpec(section.Guid);
                    ...
                    case CondType.None: // vapour is what it is.
                        if (section == column.MainTraySection)
                        {
                            Specification s = column.Specs.GetRefluxRatioSpec(section.Guid);
                            double rr;
                            if (s is not null)
                            {
                                rr = s.specvalue;
            ...
            foreach (TraySection section in column.TraySections)
            {
                ...
                {
                    Specification refluxratiospec = null;
                    switch (section.CondenserType)
                    {
                        case CondType.Partial:
                        case CondType.Subcooled:
        ...
        public void UpdatePorts(bool FlashAllPorts = false)
        {
            ...
            foreach (TraySection traySection in column.TraySections)
            {
                foreach (Tray tray in traySection)
                {
                    ...
                    foreach (Port_Material port in tray.Ports)
                        if (!port.IsFlowIn)
                            port.MolarFlow_.BaseValue /= column.feedRatio;
            ...
            foreach (PumpAround pa in column.PumpArounds)//Only need to update return streams
            {
                ...
                pa.StreamOut.Port.SetPortValue(ePropID.P, pa.StreamIn.Port.P, SourceEnum.UnitOpCalcResult);
                pa.StreamOut.Port.SetPortValue(ePropID.H, pa.ReturnEnthalpy, SourceEnum.UnitOpCalcResult);
                pa.StreamOut.Port.SetPortValue(ePropID.MOLEF, pa.StreamIn.Port.MolarFlow, SourceEnum.UnitOpCalcResult);
                pa.StreamOut.Port.Flash();
        ...
        public void SetBasicValues(Column column)
        {

            for (int i = 0; i < column.TraySections.Count; i++)
            {
                TraySection ts = column.TraySections[i];
           ...
           /* foreach (StreamMaterial stream in column.ConnectingNetFlows)
            {
                stream.Port.Clear();
            }

            foreach (StreamMaterial stream in column.ConnectingStreamsAll)
            {
                stream.Port.Clear();
            }*/
            ...
            foreach (TraySection traysection in column.TraySections)
            {
                foreach (Tray tray in traysection)
                {
                    if (tray.feed != null & tray.feed.cc != null && tray.feed.IsFullyDefined)
        ...
        public void EstimateTs(Column column, ColumnKMethod KMethod)
        {
        ...
        public void EstimateTs2(Column column, ColumnKMethod TMethod)
        {
            if (this.column is null)
                return;
        ...
        public void ReEstimateStripFactors()
        {
            ...
            for (int i = 0; i < no; i++)
            {
                ConnectingStream st = column.LiquidStreams.StreamList[i];
                if (st.engineDrawTray != null)
            ...
            {
                ConnectingStream st = column.VapourStreams.StreamList[i1];
                if (st.engineDrawTray != null)
            ...
            {
                PumpAround pa = column.PumpArounds.Pas[i2];
                if (pa.drawTray != null)
                    pa.DrawFactor = pa.MoleFlow * column.feedRatio / pa.drawTray.L;
            }
        }
        ...
        public bool CalcInitialTs(Column column)
        {
        ...
        public bool UpdateFactors(double[] grads, double gradientfactor = double.NaN)
        {
            ...
            {
                case ColumnSFUpdateMethod.Rapid:
                    //for (int i = 0; i < gradmod.Length; i++)
                    //    gradmod[i] *= gradientfactor;
                    break;

                case ColumnSFUpdateMethod.Excel:
                    for (int i = 0; i < gradmod.Length; i++)
                        gradmod[i] *= gradientfactor;
                    break;

                case ColumnSFUpdateMethod.Modified:
                ...
                case ColumnSFUpdateMethod.Simple:

                    for (int i = 0; i < gradmod.Length; i++)
                        gradmod[i] *= gradientfactor;
                    break;

                case ColumnSFUpdateMethod.Delayed:
                    if (delaycount < 10)
                    {
                        gradientfactor = 0.1;
                        deltacount++;
            ...
            foreach (ConnectingStream stream in column.LiquidStreams)
            {
                deltas[deltacount] = gradmod[deltacount] * JDelta * gradientfactor;
            ...
            foreach (ConnectingStream stream in column.VapourStreams)
            {
                deltas[deltacount] = gradmod[deltacount] * JDelta * gradientfactor;
        ...
        public void Update_LSS_LFactor(Column cd)
        {
            TraySection section;
            Tray tray;
        ...
        public void Update_VSS_VFactor(Column cd)
        {
            TraySection section;
            Tray tray;
        ...
        public void ClearTRD()
        {
            ...
            {
                PumpAround p = this.column.PumpArounds.Pas[i];
            ...
            for (int comp = 0; comp < NoComps; comp++)
            {
                if (column.ConnectingDraws.Count > 0)
                {
                    ...
                    {
                        ConnectingStream stream = column.LiquidStreams.StreamList[i];
        ...
        public bool SolveComponentBalance(Column column)
        {
            ...
            for (int comp = 0; comp < NoComps; comp++)
            {
                if (column.ConnectingDraws.Count > 0)
                {
                    ...
                    foreach (ConnectingStream stream in column.LiquidStreams)  // do liquid draws // TrDMatrix[Comp][Down][Across]
                    {
                        TraySection dcts_Draw = stream.EngineDrawSection;
                        TraySection dcts_Return = stream.EngineReturnSection;
                    ...
                    {
                        ConnectingStream stream = column.VapourStreams.StreamList[i];
                ...
                if (column.ConnectingNetFlows.Count > 0)
                {
                    ...
                    {
                        ConnectingStream stream = column.ConnectingNetFlows.LiquidStreams.StreamList[i];
                        TraySection dcts_Draw = stream.EngineDrawSection;
                        TraySection dcts_Return = stream.EngineReturnSection;
                    ...
                    {
                        ConnectingStream stream = column.ConnectingNetVapourStreams.StreamList[i];
                        TraySection Draw_Section = stream.EngineDrawSection;
                        TraySection Return_Section = stream.EngineReturnSection;
        ...
        public bool SolveComponentBalanceNew(Column column)
        {
            int countstages, CurrentStage;
            double TrayT, TrayPlusOneT = 0;
            double waterfact;
            ...
            for (int comp = 0; comp < NoComps; comp++)
            {
                if (column.ConnectingDraws.Count > 0)
                {
                    ...
                    foreach (ConnectingStream stream in column.VapourStreams)  // do vapour draws
                    {
                        TraySection Draw_Section = stream.EngineDrawSection;
                        TraySection Return_Section = stream.EngineReturnSection;
                        Tray DrawSectionTray;
                ...
                if (column.ConnectingNetFlows.Count > 0)
                {
                    ...
                    foreach (ConnectingStream stream in column.ConnectingNetVapourStreams)  // do vapour draws
                    {
                        TraySection Draw_Section = stream.EngineDrawSection;
                        TraySection Return_Section = stream.EngineReturnSection;
                        Tray drawtray = stream.engineDrawTray;
        ...
        public void DoCondenserCalcs(Column column)
        {
            double CondT;
        ...
        public void UpdateTrayEnthalpyBalances()  // Inside Loop // Should be non-rigourous inside loop.
        {
            ...
            {
                PumpAround p = column.PumpArounds.Pas[i];
                ...
                if (pasection >= 0 && drawtray >= 0 && pasection < column.TraySections.Count)
                {
                    ...
                    Specification s = column.Specs.GetActiveSpecs().GetPAEnergySpec(p.Guid);
                    if (s is null)
            ...
            for (int i = 0; i < IterCount; i++)
            {
                ConnectingStream st = column.VapourStreams.StreamList[i];
            ...
            for (int i = 0; i < IterCount; i++)
            {
                ConnectingStream st = column.ConnectingNetLiquidStreams.StreamList[i];
                TraySection dcts_draw = st.EngineDrawSection;
                TraySection dcts_ret = st.EngineReturnSection;
            ...
            for (int i = 0; i < IterCount; i++)
            {
                ConnectingStream st = column.ConnectingNetVapourStreams.StreamList[i];
                TraySection dcts_draw = st.EngineDrawSection;
                TraySection dcts_ret = st.EngineReturnSection;
        ...
        public bool CreateJacobianSF(Column column, ref enumCalcResult cres) // strip factor
        {
        ...
        {
            enumInverseMethod method = enumInverseMethod.Alglib;
        ...
        public bool UpdateFactorsOld(double[] gradients, double gradientfactor = 1)
        {
            ...
            foreach (ConnectingStream stream in column.LiquidStreams)
            {
                TraySection drawsection = stream.EngineDrawSection;
