...
namespace ModelEngine
{
    public static class RK
    {
        ...
        public static Pressure PSat(Components cc, Temperature T)
        {
            ...
            KappaAlphaRK(cc, P0, T,ref kappa, ref alpha, ref ac, ref a, ref b, ref A, ref B);

            amix = CalcAmix(cc, cc.MoleFractions, a, out aimix, kijm, ref aij);
            ...
            for (int i = 1; i <= 50; i++)
            {
                ...
                fugl = P0 * Math.Exp(lnPhiTotal(ZMin, Am, Bm));
                fugV = P0 * Math.Exp(lnPhiTotal(ZMax, Am, Bm));
                ...
                {
                    Pnew = P0 * ((ZMax - ZMin) - Math.Log(fugV / fugl)) / (ZMax - ZMin);
        ...
        public static ThermoProps ThermoBulk(Components cc, Pressure P, Temperature T, double[] X, enumFluidRegion state, double IdealEnthalpy, double IdealEntropy)
        {
            ...
            KappaAlphaRK(cc, P, T, ref kappa, ref alpha, ref ac, ref a, ref b, ref A, ref B);

            amix = CalcAmix(cc, X, a, out aimix, kijm, ref aij);
            ...
            dadt = -0.5 * amix / T; // * CalcDADT_RK(cc, T, alpha, aij);
            ...
            //H_Higm = (Rgas * T * (Z - 1) + (T * dadt - amix) / (2 * Math.Sqrt(2) * bmix) * Math.Log((Z + (Math.Sqrt(2) + 1) * Bm) / (Z - (Math.Sqrt(2) - 1) * Bm)));
            //S_Sigm = (Rgas * Math.Log(Z - Bm) + dadt / (2 * Math.Sqrt(2) * bmix)) * Math.Log((Z + (Math.Sqrt(2) + 1) * Bm) / (Z - (Math.Sqrt(2) - 1) * Bm));
        ...
        public static ThermoProps[] ThermoMix(Components cc, Temperature T, Pressure P, enumFluidRegion state, double IdealEnthalpy, double IdealEntropy)
        {
            ...
            dadt = CalcDADT_RK(cc, cc.MoleFractions, T, alpha, aij);
        ...
        public static double[] LnKmix(Components cc,Pressure P, Temperature T, double[] X, double[] Y, out enumFluidRegion state)
        {
            ...
            double AmixLiq = CalcAmix(cc, X, A, out AimxLiq, kijm, ref aij);
            ...
            double AmixVap = CalcAmix(cc, Y, A, out AimxVap, kijm, ref aij);
            double BmixVap = CalcBmix(cc, Y, B, lijm);

            CalcCompressibility(AmixVap, BmixVap, ref ZMin, ref ZMax);

            statevap = CommonRoutines.CheckState(ZMin, ZMax, T, TCritMixVap, P, PCritMixVap);

            ZVap = ZMax;

            LnPhi(cc, ZLiq, AmixLiq, BmixLiq, AimxLiq, B, lnPhiL1inf);
            LnPhi(cc, ZVap, AmixVap, BmixVap, AimxVap, B, lnPhiV1inf);
        ...
        public static enumFluidRegion GetFluidState(Components cc, Temperature T, Pressure p)
        {
            ...
            double Amix = CalcAmix(cc, cc.MoleFractions, a, out _, kijm, ref aij);
        ...
        public static double[] LnFugMix(Components cc, Pressure Pp, Temperature T, enumFluidRegion state)
        {
            int NoComps = cc.ComponentList.Count;
            double[,] kijm = new double[NoComps, NoComps];
            double[,] lijm = new double[NoComps, NoComps];
            ...
            double amix = CalcAmix(cc, cc.MoleFractions, a, out Aimx, kijm, ref aij);
            double bmix = CalcBmix(cc, cc.MoleFractions, b, lijm);

            var Am = amix * P / Math.Pow(Rgas * T, 2);
            var Bm = P * bmix / (Rgas * T);

            CalcCompressibility(Am, Bm, ref ZMin, ref ZMax);

            if (state == enumFluidRegion.Liquid)
            {
                LnPhi(cc, ZMin, Am, Bm, A, B, lnPhiMix);
            }
            else
            {
                LnPhi(cc, ZMax, Am, Bm, A, B, lnPhiMix);
            }

            return lnPhiMix;
        }

        public static double KmixInfDil(Components cc1, Temperature T, Pressure P, int I_Inf)
        {
            ...
            double Amix = CalcAmix(cc, cc.MoleFractions, a, out double[] Aimx, kijm, ref aij);
            double Bmix = CalcBmix(cc, cc.MoleFractions, b, lijm);
        ...
        public static void KappaAlphaRK(Components cc, Pressure Pp, Temperature T, ref double[] kappa, ref double[] alpha,
            ref double[] ac, ref double[] a, ref double[] b, ref double[] A, ref double[] B)
        {
            ...
            for (int I = 0; I < NoComps; I++)
            {
                bc = cc[I];
                Pr = P / bc.CritP.MPa;
                Tr = T / bc.CritT.Kelvin;
                CritP = bc.CritP.MPa;
                CritT = bc.CritT.Kelvin;

                // kappa[I] = 1;  //RK
                alpha[I] = Tr.Pow(-0.5);
                ac[I] = 0.42748 * Rgas * Rgas * CritT.Pow(5 / 2) / CritP;
                a[I] = ac[I] * alpha[I];
                //a[I] = (0.4278 / (CritP * (Tr.Pow(2.5)))).Pow(0.5);
                A[I] = a[I] * P / (Rgas * T).Pow(2);
                b[I] = 0.08664 * Rgas * CritT / CritP;
                B[I] = b[I] * P / Rgas / T;
            }
        }
        ...
        public static double CalcAmix(Components cc, double[] XY, double[] A, out double[] Aimix, double[,] kijm, ref double[,] Aij)
        {
            //
        ...
        public static double CalcDADT_RK(Components cc, double[] XY, Temperature T, double[] alpha, double[,] aij)
        {
            ...
            for (int I = 0; I < alpha.Length; I++)
            {
                ...
                {
                    bcJ = cc[J];
                    dadt_i[I] += -aij[I, J] / (2 * Math.Sqrt(T)) * ((1 / Math.Sqrt(alpha[J] * bcJ.CritT)
                        + 1 / Math.Sqrt(alpha[I] * bcI.CritT))) * XY[J];
                }
                dadt += dadt_i[I] * XY[I];
            }
            return dadt;
        ...
        public static double CalcDADTAprrox(Components cc, Temperature Tk, double TcritKMix, double amix, double kappamix, double alphamix)
        {
            double dadt;

            dadt = -amix * Tk.Pow(-0.5) * kappamix * TcritKMix.Pow(-0.5) * alphamix.Pow(-0.5);
        ...
        public static void LnPhi(Components cc, double Z, double A, double B, double[] Ai, double[] Bi, double[] lnPhi_i)
        {
            ...
            for (int i = 0; i < N; i++)
            {
                ...
                lnPhi_i[i] = (Bi_Bm * (Z - 1) - Math.Log(Z - B) + A / B * (2 * Ai_Am - Bi_Bm) * Math.Log(Z / (Z + B))); // RK / SRK
            }
        }
        ...
        public static ThermoDifferentialPropsCollection DifferentialProps(Components cc, Pressure P, Temperature T, enumFluidRegion state, double IdealCp)
        {
            ...
            double v = Z * Rgas * T / P;
            var dp_dv_t = -Rgas * T / (v - bmix).Pow(2) + 2 * amix * (v + bmix) / (v * (v + bmix) + bmix * (v - bmix)).Pow(2);
            var dp_dt_v = Rgas / (v - bmix) - dadt / (v * (v + bmix) + Bm * (v - Bm));
            var dt_dp_v = 1 / dp_dt_v;
            var da_dt_p = P / Rgas.Pow(2) / T.Pow(2) * (dadt - 2 * amix / T);
            var db_dt_p = -bmix * P / (Rgas * T * T);
            var dz_dt_p = (da_dt_p * (Bm - Z) + db_dt_p * (6 * Bm * Z + 2 * Z - 3 * Bm * Bm - 2 * Bm + Am - (Z * Z))) / (3 * Z * Z + 2 * (Bm - 1) * Z + (Am - 2 * Bm - 3 * Bm * Bm));
            var dv_dt_p = Rgas / P * (T * (dz_dt_p) + Z);
            ...
            for (int I = 0; I < kappa.Length; I++)
            {
                for (int J = 0; J < kappa.Length; J++)
                {
                    aij_dd[I, J] = XY[I] * XY[J] * (1) * (ai_dd[I] * ai_dd[J] / Math.Sqrt(ai[I] * ai_dd[J])
                        ...
                        - 1 / 2 * (ai_d[I].Sqr() * ai[J].Sqr() / ai_dd[I].Pow(3 / 2) + ai_d[J].Pow(2) * Math.Sqrt(ai[I]) / ai_dd[J].Pow((3 / 2))));
                }
            }
