...
namespace Units
{
    ...
    ///
    [Serializable]
    internal class DrawReformer : DrawRectangle, ISerializable
    {
        private ReformerBed reformer = new ReformerBed();

        public DrawReformer() : this(0, 0, 1, 1)
        {
        }

        public DrawReformer(int x, int y, int width, int height)
            : base()
        {
            if (width < 20)
                width = 20;
            ...
            Hotspots.Add(new Node(this, 1f, 0f, "VAPPRODUCT", NodeDirections.Down, HotSpotType.VapourDraw, HotSpotOwnerType.DrawRectangle));
            Initialize();

            this.Name = "Reformer" + Count.ToString();
            Count++;
        }

        public override void CreateFlowsheetUOModel()
        {
            this.Flowsheet.Add(reformer);
            Node Inlet, Outlet;

            Inlet = Hotspots["FEED"];
            Outlet = Hotspots["VAPPRODUCT"];

            Inlet.PortGuid = reformer.Ports["In1"].Guid;
            Outlet.PortGuid = reformer.Ports["Out1"].Guid;

            reformer.Name = this.Name;
        }
        ...
        public DrawReformer(SerializationInfo info, StreamingContext context) : base(info, context)
        {
        }
