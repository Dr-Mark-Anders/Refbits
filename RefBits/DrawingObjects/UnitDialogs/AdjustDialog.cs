...
namespace Units
{
    public partial class AdjustDialog : BaseDialog, IUpdateableDialog
    {
        private readonly AdjustObject adjust;
        ...
        internal AdjustDialog(DrawAdjust dset, DrawSignalStream streamin, DrawSignalStream streamout, DrawObject startobject, DrawObject endobject)
        {
            dset.adjust.ConnectionType = ConnectionType.notvalid;
            if (startobject is not null && endobject is not null)
            {
                startObject = startobject;
                endObject = endobject;
                dset.adjust.ConnectionType = ConnectionType.in_out;
            }
            else if (startobject is null)
            {
                startObject = endobject;
                endObject = endobject;
                dset.adjust.ConnectionType = ConnectionType.outout;
            }
            else
            {
                startObject = startobject;
                endObject = startobject;
                dset.adjust.ConnectionType = ConnectionType.inin;
            }
        ...
        private void AdjustDialog_Load(object sender, System.EventArgs e)
        {
            ...
            treeView1.ExpandAll();
            treeView1.SelectedNode = treeView1.Nodes[0].Nodes[adjust.MVpropid.ToString()];
            ...
            treeView2.ExpandAll();
            treeView2.SelectedNode = treeView2.Nodes[0].Nodes[adjust.CVpropid.ToString()];

            if (adjust != null && adjust.Dataout != null)
                uom2.UOMprop = adjust.Dataout;

            txtTargetValue.Text = adjust.FinalCV.ToString();
