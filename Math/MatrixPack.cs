// Lutz Roeders's .NET Mapack, August 2001, adapted from the Java Matrix Package.
// Copyright (C) 2000-2001 Lutz Roeder. All rights reserved.
...
namespace MatrixAlgebra
{
    ...
    public interface IMatrix
    {
        ...
        /// <returns>Sum of the diagonal elements.</return  s>
        double Trace { get; }
        ...
        /// <summary>return  s the LHS solution vetor if the matrix is square or the least squares solution otherwise.</summary>
        IMatrix Solve(IMatrix rhs);

        /// <summary>return  s the cholesky decomposition for this matrix.</summary>
        ICholeskyDecomposition GetCholeskyDecomposition();

        /// <summary>return  s the LU decomposition for this matrix.</summary>
        ILuDecomposition GetLuDecomposition();

        /// <summary>return  s the singular value decomposition for this matrix.</summary>
        ISingularValueDecomposition GetSingularValueDecomposition();

        /// <summary>return  s the QR decomposition for this matrix.</summary>
        IQrDecomposition GetQrDecomposition();

        /// <summary>return  s the eigenvalue decomposition for this matrix.</summary>
    ...
    ///		If the matrix is not symmetric or positive definite, the constructor return  s a partial
    ///		decomposition and sets two public  variables that can be queried using   the
    ...
    public interface ICholeskyDecomposition
    {
        ...
        /// <return  s>Matrix <c>X</c> so that <c>L * L' * X = B</c>.</return  s>
        ...
        /// <summary>return  s <see langword="true"/> if the matrix is positive definite.</summary>
        Boolean IsPositiveDefinite { get; }

        /// <summary>return  s <see langword="true"/> if the matrix is symmetric.</summary>
        Boolean IsSymmetric { get; }

        /// <summary>return  s the left triangular factor <c>L</c> so that <c>A = L * L'</c>.</summary>
    ...
    ///   linear equations. This will fail if <see cref="IsNonSingular"/> return  s <see langword="false"/>.
    /// </remarks>
    public interface ILuDecomposition
    {
        /// <summary>return  s if the matrix is non-singular.</summary>
        bool IsNonSingular { get; }

        /// <summary>return  s the determinant of the matrix.</summary>
        double Determinant { get; }

        /// <summary>return  s the lower triangular factor <c>L</c> with <c>A=LU</c>.</summary>
        IMatrix LowerTriangularFactor { get; }

        /// <summary>return  s the lower triangular factor <c>L</c> with <c>A=LU</c>.</summary>
        IMatrix UpperTriangularFactor { get; }

        /// <summary>return  s the pivot permuation vector.</summary>
        double[] PivotPermutationVector { get; }

        /// <summary>Solves a set of equation systems of type <c>A * X = B</c>.</summary>
        /// <param name="rhs">Right hand side matrix with as many rows as <c>A</c> and any number of columns.</param>
        /// <return  s>Matrix <c>X</c> so that <c>L * U * X = B</c>.</return  s>
        IMatrix Solve(IMatrix rhs);
    }
    ...
    ///   This will fail if <see cref="IsFullRank"/> return  s <see langword="false"/>.
    /// </remarks>
    public interface IQrDecomposition
    {
        ...
        /// <summary>return  s the upper triangular factor <c>R</c>.</summary>
        IMatrix UpperTriangularFactor { get; }

        /// <summary>return  s the orthogonal factor <c>Q</c>.</summary>
        IMatrix OrthogonalFactor { get; }

        /// <summary>Least squares solution of <c>A * X = B</c></summary>
        /// <param name="rhs">Right-hand-side matrix with as many rows as <c>A</c> and any number of columns.</param>
        /// <return  s>A matrix that minimized the two norm of <c>Q * R * X - B</c>.</return  s>
    ...
    {
        /// <summary>return  s the condition number <c>max(S) / min(S)</c>.</summary>
        double Condition { get; }

        /// <summary>return  s the Two norm.</summary>
        double Norm2 { get; }

        /// <summary>return  s the effective numerical matrix rank.</summary>
    ...
    {
        /// <summary>return  s the real parts of the eigenvalues.</summary>
        double[] RealEigenvalues { get; }

        /// <summary>return  s the imaginary parts of the eigenvalues.</summary>
        double[] ImaginaryEigenvalues { get; }

        /// <summary>return  s the eigenvector matrix.</summary>
        IMatrix EigenvectorMatrix { get; }

        /// <summary>return  s the block diagonal eigenvalue matrix.</summary>
    ...
    public class Matrix : IMatrix
    {
        ...
        {
            this.rows = data1.GetUpperBound(0) + 1;
            this.columns = data1.GetUpperBound(1) + 1;
        ...
        /// <summary>return  s the number of columns.</summary>
        public int Rows
        {
            get { return rows; }
        }
        ...
        /// <summary>return  s a sub matrix extracted from the current matrix.</summary>
        ...
        /// <summary>return  s the transposed matrix.</summary>
        ...
        /// <summary>return  s the One Norm for the matrix.</summary>
        /// <value>The maximum column sum.</value>
        public double Norm1
        {
            get
        ...
        /// <summary>return  s the Infinity Norm for the matrix.</summary>
        /// <value>The maximum row sum.</value>
        ...
        /// <summary>return  s the Frobenius Norm for the matrix.</summary>
        /// <value>The square root of sum of squares of all elements.</value>
        ...
        */
        ...
        /// <summary>return  s the trace of the matrix.</summary>
        /// <return  s>Sum of the diagonal elements.</return  s>
        public double Trace
        {
            get
            {
        ...
        /// <summary>return  s a matrix filled with random values.</summary>
        ...
        /// <summary>return  s a diagonal matrix of the given size.</summary>
        ...
        /// <summary>return  s the matrix in a textual form.</summary>
            ...
            public SingularValueDecomposition(Matrix A)
            {
                ...
                for (int k = 0; k < Math.Max(nct, nrt); k++)
                {
                    ...
                    for (int j = k + 1; j < n; j++)
                    {
                        ...
                        // Place the k-th row of A int o e for the subsequent calculation of the row transformation.
                        e[j] = a[k][j];
                    }
        ...
        private class EigenvalueDecomposition : IEigenvalueDecomposition
        {
            ...
            private void tred2()
            {
                ...
                // Handbook for Auto. Comp., Vol.ii-Linear Algebra, and the corresponding Fortran subrout ine in EISPACK.
                for (int j = 0; j < n; j++)
                    d[j] = V[n - 1, j];
            ...
            private void orthes()
            {
                ...
                // Handbook for Auto. Comp., Vol.ii-Linear Algebra, and the corresponding Fortran subrout ines in EISPACK.
                int low = 0;
                int high = n - 1;
                ...
                for (int m = high - 1; m >= low + 1; m--)
                {
                    if (H[m, m - 1] != 0.0)
                    {
                        ...
                        for (int j = m; j <= high; j++)
                        {
                            ...
                            // double  division avoid  s possible underflow.
                            g = (g / ort[m]) / H[m, m - 1];
                            for (int i = m; i <= high; i++)
                                V[i, j] += g * ort[i];
                        }
            ...
            private void hqr2()
            {
                ...
                // Vol.ii-Linear Algebra, and the corresponding  Fortran subrout ine in EISPACK.
                int nn = this.n;
                int n = nn - 1;
                int low = 0;
                int high = nn - 1;
                ...
                // out er loop over eigenvalue index
                int iter = 0;
                while (n >= low)
                {
